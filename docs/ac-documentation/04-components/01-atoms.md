...
Defined here (atoms layer); responsive cascade rules live in [[../05-framework|framework.css]].
...
Migrated here from [[../05-framework|framework.css]] on 2026-04-30. Used to visually separate stacked `PageSection` children.
...
Don't add page-level chrome here. That's [[../05-framework|framework.css]]. Don't add bespoke per-page styles — those belong in app layers.

...
Eight named strata. **Use a stratum, not a magic number.** If you're typing `z-index: 9999`, you're stepping outside the system — there's exactly one place this is legitimate ([[../05-framework|framework]]'s `.ac-exit-preview`, which is intentionally above everything).

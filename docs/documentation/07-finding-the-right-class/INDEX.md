...
→ [[05-framework/INDEX|framework.css]]. `PageSection.jsx` provides the children — they own their own gap, don't add `mt-*`.
...
→ `.ac-overlay` / `.ac-overlay-sheet` / `.ac-overlay-close` in [[05-framework/INDEX|framework.css]]. JS lock handles `body { overflow: hidden }`.
...
→ `.ac-embla-*` family in [[05-framework/INDEX|framework.css]]. Embla-driven.
...
→ [[05-framework/INDEX|framework.css]].
...
1. **Pick the right tier.** Token? Utility? Component? Framework? App-layer? Look at [[01-cascade/INDEX|cascade]] to be sure.
...
Open [[INDEX|the pillar INDEX]] for the five-tier overview, then [[01-cascade/INDEX|cascade]] for the load order, then the tier doc for the specific question.
